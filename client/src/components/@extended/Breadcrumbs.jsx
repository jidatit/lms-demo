...
import menuItems, { agentMenuItems, contributorMenuItems, groupLeaderMenuItems, subscriberMenuItems } from 'menu-items';
...
export default function Breadcrumbs({
  ...
  const determineDashboardPath = () => {
    if (location.pathname.includes('contrib_dashboard')) return '/contrib_dashboard/home';
    if (location.pathname.includes('subscriber_dashboard')) return '/subscriber_dashboard';
    if (location.pathname.includes('groupleader_dashboard')) return '/groupleader_dashboard/home';
    if (location.pathname.includes('agent_dashboard')) return '/agent_dashboard/profile/user/personal';
    return '/dashboard/home'; // Default
  };

  const dashboardPath = determineDashboardPath();
  ...
  if (!custom && main && main.type === 'collapse' && main.breadcrumbs === true) {
    ...
    breadcrumbContent = (
      <MainCard
        border={card}
        sx={card === false ? { mb: 3, bgcolor: 'transparent', ...sx, pt: 2 } : { mb: 3, ...sx, pt: 2 }}
        {...others}
        content={card}
        boxShadow={false}
      >
        ...
        >
          <Grid item>
            <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon} sx={{ marginTop: '2px' }}>
              <Typography
                component={Link}
                to={dashboardPath}
                variant="body1"
