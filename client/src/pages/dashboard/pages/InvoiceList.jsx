    ...
    <MainCard content={false}>
      <Box sx={{ p: 2.5, pb: 0, width: '100%' }}>
        ...
        >
          ...
          <Button
            ...
            onClick={() => {
              // console.log('clicked');
              navigation('/dashboard/invoices/create');
            }}
          >
            {'Create Invoice'}
          </Button>
      ...
      <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between" sx={{ padding: 2.5 }}>
        ...
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <CSVExport
            {...{ data: table.getSelectedRowModel().flatRows.map((row) => row.original), headers, filename: 'invoice-list.csv' }}
          />
        </Stack>
      </Stack>
      <ScrollX>
...
export default function InvoiceList() {
      ...
      if (originalRow) {
        ...
        navigation(`details?invoiceId=${originalRow.invoiceNumber}&status=${originalRow.status}`, {
          state: {
            ...
            // cashierInfo: {
            //   name: bundlePurchaseDetails.createdByUser?.profile?.businessName || '',
            //   email: bundlePurchaseDetails.createdByUser?.email || '',
            //   phone: bundlePurchaseDetails.createdByUser?.profile?.phoneNumber || '',
            //   address: bundlePurchaseDetails.createdByUser?.profile?.businessAddress || '',
            // },
  ...
  useEffect(() => {
    if (selectedExternalInvoice) {
      ...
      if (externalInvoice) {
        // console.log(`externalInvoice`, externalInvoice);
  ...
  const handleEditClick = (invoiceData) => {
    if (invoiceData.invoiceType === 'external') {
      ...
      navigation(`/dashboard/invoices/edit?invoiceId=${invoiceData.invoiceNumber}`, {
        state: {
          ...
          customerInfo: {
            name: invoiceData.company?.name || '',
            email: invoiceData.company?.id || '',
            address: invoiceData.company?.address || '',
            vatNumber: invoiceData.company?.vatNumber || ''
          },
          cashierInfo: {
            name: invoiceData?.createdByUser?.profile?.businessName || '',
            email: invoiceData?.createdByUser?.email || '',
            phone: invoiceData?.createdByUser?.profile?.phoneNumber || '',
            address: invoiceData?.createdByUser?.profile?.businessAddress || '',
          },
