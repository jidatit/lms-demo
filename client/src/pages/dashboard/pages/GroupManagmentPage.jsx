...
import { activateLmsGroup, deactivateLmsGroup, deleteLmsGroup } from '../utils/groupApiUtils';
import { FaDeleteLeft } from 'react-icons/fa6';
import { MdDelete } from 'react-icons/md';
import ConfirmDialog from '../components/ConfirmDialog';
import { useCompanies, useCreateCompany } from 'api/queries/companies';
...
const GroupManagmentPage = () => {
  ...
  // Updated handleGroupSubmit function
  ...
  const handleNumberChange = (e) => {
    ...
    const limitedNum = Math.min(Math.max(num, 1), 15);
  ...
  const handleSubmitGroupLeader = async (event) => {
    ...
    try {
      ...
      setGroupLeadersData(response.data);
    } catch (error) {
  ...
  const handleAddExistingUserToGroup = async () => {
    try {
      setLoading(true);
      await addGroupMember({
        groupId: SelectedGroupId,
        userId: existingUserId, // We got this from error response
        role: 'groupLeader'
      });

      toast({ message: 'Existing user added to group successfully!', type: 'success' });
      setGroupLeadersData((prev) => [...prev, { id: existingUserId, email: existingUserEmail, role: 'groupLeader' }]);
    } catch (err) {
      toast({ message: err.message || 'Failed to add existing user to group', type: 'error' });
    } finally {
      setLoading(false);
  ...
  // Updated handleConfirm function (remove old activate/deactivate calls, use toggle for status changes)
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Refresh your list (assuming useGetGroups is used elsewhere, it will refetch due to invalidation)
  ...
  return (
    <>
      ...
      >
        <DialogTitle>
          <Box display="flex" alignItems="center" justifyContent="space-between">
            Bulk Add and Invite Staff
            <Typography variant="caption" color="text.secondary">
              {users.filter((user) => user.firstName || user.lastName || user.email).length} of {numberOfUsers} users filled
            </Typography>
          </Box>
        </DialogTitle>
        <DialogContent>
          <Box mb={3} mt={1}>
            ...
            <TextField
              ...
              size="small"
              sx={{ width: 150 }}
              helperText="Maximum 15 users can be added at once"
              FormHelperTextProps={{
          ...
          <DialogContentText sx={{ mb: 2 }}>
            To add Staff, please provide their First Name, Last Name, and Email. All fields are required.
          </DialogContentText>

          <Box sx={{ maxHeight: 500, overflowY: 'auto', border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
            <Box
              display="grid"
            ...
            <Box sx={{ p: 2 }}>
              {users.map((user, index) => {
                const hasAnyData = user.firstName || user.lastName || user.email;
                const isComplete = user.firstName && user.lastName && user.email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(user.email);
                  ...
                  <Grid container key={index} spacing={2} sx={{ mb: 2 }}>
                    <Grid item xs={12} sm={4}>
                      <TextField
                        ...
                        size="small"
                        required={hasAnyData}
                        error={hasAnyData && !user.firstName}
                        helperText={hasAnyData && !user.firstName ? 'Required' : ''}
                      />
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <TextField
                        ...
                        size="small"
                        required={hasAnyData}
                        error={hasAnyData && !user.lastName}
                        helperText={hasAnyData && !user.lastName ? 'Required' : ''}
                      />
                    </Grid>
                    <Grid item xs={12} sm={4}>
                      <TextField
                        ...
                        size="small"
                        required={hasAnyData}
                        error={hasAnyData && (!user.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(user.email))}
                        helperText={
                          hasAnyData && !user.email
                            ? 'Required'
                            : hasAnyData && user.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(user.email)
                              ? 'Invalid email format'
                              : ''
                        }
                        InputProps={{
                          endAdornment: isComplete && <Box sx={{ color: 'success.main', display: 'flex', alignItems: 'center' }}>✓</Box>
                        }}
                      />
                    </Grid>
                  </Grid>
        ...
        <DialogActions sx={{ padding: '16px', gap: 1 }}>
          <Button onClick={handleCloseModalBulk} color="error" disabled={loading}>
            Cancel
          </Button>
          <Button
            type="submit"
            ...
            disabled={
              loading ||
              users.filter((user) => user.firstName && user.lastName && user.email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(user.email))
                .length === 0
            }
          ...
          >
            {loading
              ? 'Processing...'
              : `Add ${users.filter((user) => user.firstName && user.lastName && user.email).length} Staff${users.filter((user) => user.firstName && user.lastName && user.email).length !== 1 ? 's' : ''}`}
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog
      ...
      >
        <DialogTitle>
          <Box display="flex" alignItems="center" justifyContent="space-between">
            Bulk Add and Invite Group Leaders
            <Typography variant="caption" color="text.secondary">
              {users.filter((user) => user.firstName || user.lastName || user.email).length} of {numberOfUsers} users filled
            </Typography>
          </Box>
        ...
        <DialogContent>
          <Box mb={3} mt={1}>
            ...
            <TextField
              ...
              size="small"
              sx={{ width: 150 }}
              helperText="Maximum 15 group leaders can be added at once"
              FormHelperTextProps={{
                sx: { fontSize: '0.75rem', color: 'text.secondary' }
              }}
            />
          ...
          <DialogContentText sx={{ mb: 2 }}>
            To add Group Leaders, please provide their First Name, Last Name, and Email. All fields are required.
          </DialogContentText>
        ...
        <DialogActions sx={{ padding: '16px', gap: 1 }}>
          ...
          >
            {loading
              ? 'Processing...'
              : `Add ${users.filter((user) => user.firstName && user.lastName && user.email).length} Group Leader${users.filter((user) => user.firstName && user.lastName && user.email).length !== 1 ? 's' : ''}`}
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog open={showAddExistingModal} onClose={() => setShowAddExistingModal(false)}>
        <DialogTitle>User Already Exists</DialogTitle>
        <DialogContent>
          <Typography>
            The user <strong>{existingUserEmail}</strong> already exists in the platform. Do you want to add them to this group?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setShowAddExistingModal(false)}>Cancel</Button>
          <Button variant="contained" color="primary" onClick={handleAddExistingUserToGroup}>
            Add to Group
          </Button>
        </DialogActions>
      </Dialog>
